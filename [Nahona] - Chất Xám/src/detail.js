...
function execute(url) {
    ...
    if (response.ok) {
        ...
        let descriptionMeta = doc.select(".entry-content div").html();
        ...
        let author = doc.select(".entry-content").select("p").get(1).text();
        let novelCategory = doc.select('meta[property="og:novel:category"]').attr("content");
        let status = doc.select(".entry-content").select("p").get(2).text();
