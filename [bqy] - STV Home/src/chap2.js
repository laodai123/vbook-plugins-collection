function execute(url) {
    ...
    el1 = el1.html()
    console.log(el1)
    // el1 = Html.clean(el1, ["i","br"]).replace(/<i>/g, "").replace(/<\/i>/g, "").replace(/Đọc trên web để có chất lượng dịch cao và ủng hộ website./g, "").replace("@Bạn đang đọc bản lưu trong hệ thống \n<br>","").replace(/  /g, " ")
    // el1 = Html.parse(el1)
    el2 = el1.replace(/\" v\=(.*?)\<\/i\>/g, "</i>").replace(/\<i (.*?) t\=\"/g, "<i>").replace(/Đọc trên web để có chất lượng dịch cao và ủng hộ website./g, "").replace(/@Bạn đang đọc bản lưu trong hệ thống/g,"").replace(/\" onclick\=\"pr\(this\);\">	<\/i>/g,"</i>").replace(/ /g,"")
    el2 = el2.replace(/\“ /g,"\“")
